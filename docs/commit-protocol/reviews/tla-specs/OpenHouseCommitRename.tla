...
(*                                                                           *)
(* The constant RenameGuard toggles the fix.  In the code the guard is two   *)
(* steps: the HTS service checks the caller's expected metadataLocation      *)
(* against the row it reads, then issues an UPDATE conditional on that       *)
(* row's @Version, bumping @Version atomically; a mismatch updates 0 rows    *)
(* -> 409 conflict -> the renamer reloads and retries.  The model            *)
(* collapses the two steps into the single atomic guard catLoc = rBase,      *)
(* which is sound because every UserTableRow write bumps @Version            *)
(* (renameTableId is the only JPQL UPDATE on the row; all other writes are   *)
(* versioned JPA saves).  The guarded model covers token-carrying renames    *)
(* only: a rename with no expected base (an old client, or the               *)
(* INITIAL_VERSION fallback for legacy tables with no persisted              *)
(* tableLocation) is guarded only between the HTS service's own read and     *)
(* UPDATE, and is not modeled.                                               *)
(*                                                                           *)
