# Design & Estimate: Iceberg REST-Catalog-Native Commit Protocol for OpenHouse
...
All OpenHouse paths relative to `/home/user/openhouse` (HEAD `2a9dac8`), Iceberg reference paths relative to `/home/user/iceberg` (apache/iceberg 1.5.2.x line, matching the `com.linkedin.iceberg` 1.5.2 fork the OpenHouse server builds against — `buildSrc/src/main/groovy/openhouse.iceberg-conventions-1.5.2.gradle:6-9`). Companion reports: `01-openhouse-commit-protocol.md` (current protocol), `02-snapshot-drop-bug.md` (#612 motivation), `03-iceberg-commit-deepdive.md` (REST-native reference).
...
Adopt the Iceberg REST catalog spec (`/home/user/iceberg/open-api/rest-catalog-open-api.yaml`) for the table lifecycle surface, served by the existing Tables Spring service alongside the legacy endpoints during migration:
...
Model directly on `CatalogHandlers.commit` (`/home/user/iceberg/core/src/main/java/org/apache/iceberg/rest/CatalogHandlers.java:345-388`), which the server's iceberg-core 1.5.2 fork already contains on the classpath:
...
- OpenHouse server commit core: `/home/user/openhouse/iceberg/openhouse/internalcatalog/src/main/java/com/linkedin/openhouse/internal/catalog/OpenHouseInternalTableOperations.java` (`doCommit` :250-489; merge :314-354; CAS :604-635; dedup :642-664; replicated rewrite :281-284, :420-422)
- Legacy staging/smuggling: `/home/user/openhouse/services/tables/src/main/java/com/linkedin/openhouse/tables/repository/impl/OpenHouseInternalRepositoryImpl.java` (:111-226 save; :196 COMMIT_KEY; :201-207 retry-disable; :451-475 versionCheck; :696-708 snapshot staging)
- HTS CAS (unchanged): `/home/user/openhouse/services/housetables/src/main/java/com/linkedin/openhouse/housetables/services/UserTablesServiceImpl.java` :98-127; `model/UserTableRow.java` :28; `dto/mapper/UserTableVersionMapper.java` :20-47
...
- Iceberg reference: `/home/user/iceberg/core/src/main/java/org/apache/iceberg/rest/CatalogHandlers.java` :283-388; `UpdateRequirements.java` :50-174; `UpdateRequirement.java` :38-232; `rest/RESTTableOperations.java` :105-158; `open-api/rest-catalog-open-api.yaml` :592-772, :2772-2788, :3294-3303
- Legacy client to retire: `/home/user/openhouse/integrations/java/iceberg-1.2/openhouse-java-runtime/src/main/java/com/linkedin/openhouse/javaclient/OpenHouseTableOperations.java` :97-128, :142-169, :364-391, :418-464
