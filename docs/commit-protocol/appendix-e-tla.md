...
**What was abstracted away** (honestly): metadata.json contents beyond the snapshot-ID set; refs/branches; schema/property evolution; snapshot expiration (writers only append — see §5); CommitStateUnknown / ambiguous HTS responses (the ambiguous half of [protocol.md](protocol.md) §4 row S4); dedup-cache TTL and size eviction; the rename/replace/stage-create paths; multiple tables; the client/engine retry counter (retries bounded only by a global commit budget).
...
`prefix_sharedcache.cfg` (pre-fix, but all writers hitting **one** Tables-service JVM, so `failIfRetryUpdate`'s cache is shared): **no violation** (105/45 states). Because the commitKey is just the base metadata path, two writers committing from the same base get serialized by the shared cache ([Appendix B](appendix-b-code-review.md), the dedup-cache finding) — the second one is 409'd before the stale doCommit can run. This is consistent with the incident requiring multi-replica routing (or the unmodeled 5-min TTL / 1000-entry eviction) to manifest, and it explains why the bug could survive single-instance testing. It must **not** be read as a correctness argument: the model omits eviction, so the shared-cache "pass" is an artifact of these constants, and the honest production topology is the per-replica configuration that violates.
...
3. **Undefended paths not modeled**: replaceTable/RTAS, stage-create/stage-replace (no `COMMIT_KEY` — intentionally authoritative), rename (which bypasses the `@Version` CAS entirely — [Appendix B](appendix-b-code-review.md) blocking finding 3), property-only commits.
4. **Dedup cache TTL/eviction not modeled** (see §5); paths compared as atoms, so the scheme-normalization concerns ([Appendix B](appendix-b-code-review.md)) and location-reuse assumption are assumed away (though `MonotonicVersion` checks no committed location is ever reused *within* the model).
...
- **Worth doing (moderate value, small effort):** (a) add an `Expire(w)` action (writer stages a payload omitting a known snapshot) and the refined NoSnapshotLoss with an explicit-expiry ghost set — verifies the fix doesn't break legitimate expiration and sharpens the invariant to explicit-expiry-only snapshot removal; (b) model the **rename vs. commit race** ([Appendix B](appendix-b-code-review.md) blocking finding 3) — the JPQL rename bypasses `@Version`, and the same model skeleton would likely find a lost-update counterexample there, i.e. a *new* bug candidate rather than a regression check.
