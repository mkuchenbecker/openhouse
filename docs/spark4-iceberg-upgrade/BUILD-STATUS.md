...
| **3** | Spark 4.0 / **Iceberg 1.11** / Java 17 | matrix ≡ rung-2 + **DV battery** | ✅ **1637 / 28 / 32 — IDENTICAL to rung-2** (same 32 REST-first fails, 0 new/regressed). Fork bumped to `1.11.0-openhouse` (core+api patches replayed onto apache-iceberg-1.11.0; spark-4.0 stock; APP_NAME + palantir-revapi dropped as now-upstream; Java floor 17). **DV battery GREEN**: v3 merge-on-read DELETE/UPDATE/MERGE writes `deletion-vector-v1` puffin blobs (1 DV/data file), reads correct, round-trips through the server v3-write path. Finding: **1.10 already writes DVs** — see `rung3-dv/`. |
