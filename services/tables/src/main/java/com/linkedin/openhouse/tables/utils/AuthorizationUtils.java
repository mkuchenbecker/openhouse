...
public class AuthorizationUtils {
  ...
  /**
   * Checks if the acting principal is authorized to replace the table. Replace is a wholesale
   * overwrite of the table's data and metadata, so it requires the same write-path privileges as
   * any other table write, including the SYSTEM_ADMIN escalation for replica tables.
   *
   * @param tableDto
   * @param actingPrincipal
   */
  public void checkReplaceTablePrivilege(TableDto tableDto, String actingPrincipal) {
    checkTableWritePathPrivileges(tableDto, actingPrincipal, Privileges.UPDATE_TABLE_METADATA);
  }
}
