...
class CadenceBasedTableStatsCollectionAnalyzerTest {
  ...
  private CadenceBasedTableStatsCollectionAnalyzer analyzer;

  @BeforeEach
  void setUp() {
    analyzer =
        new CadenceBasedTableStatsCollectionAnalyzer(
            new CadencePolicy(TEST_SUCCESS_INTERVAL, TEST_FAILURE_INTERVAL));
  }

  @Test
  void getOperationType_isTableStatsCollection() {
  ...
  private TableDto tableWithProperty(boolean enabled) {
    return TableDto.builder()
        ...
        .tableProperties(
            Map.of(
                CadenceBasedTableStatsCollectionAnalyzer.TABLE_STATS_COLLECTION_ENABLED_PROPERTY,
