...
class OpenhouseSparkSqlExtensionsParser (delegate: ParserInterface) extends ParserInterface {
  ...
  // Spark 4.0 added parseQuery to ParserInterface; delegate it unchanged (OpenHouse only
  // intercepts its own DDL in parsePlan, everything else is passed through).
  ...
  // Spark 4.0 replaced the old parseRawDataType hook with parseRoutineParam on ParserInterface.
