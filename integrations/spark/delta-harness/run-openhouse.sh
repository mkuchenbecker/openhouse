...
# spark4-upgrade rung 2: Spark 4.0 is Scala 2.13 only. Spark 4.0.0 ships scala-library 2.13.16, so
...
# spark4-upgrade rung 2: resolve the classpath from the Spark-4.0/Scala-2.13 itest module
# (REST-first: stock iceberg-spark-runtime-4.0 + embedded OpenHouse server, no custom OH runtime).
...
# spark4-upgrade rung 2: Spark 4.0's documented --add-opens set (org.apache.spark.launcher
# JavaModuleOptions), a superset of the Spark-3.5 list used before.
OPENS=(
