...
2289/11/0 (2300), 0 fail, 0 ORC↔Parquet divergence. Branch-vs-release = no difference on any tested
behaviour (+4 cases = the fork.colDefault.* block). #251: OSS Spark DDL path unchanged (no write wiring
on the branch), SchemaParser serializes the default (no format-version argument). See
CHECKLIST-2026-07-20-test-the-branch.md, VERIFIED-RUN top block, ICEBERG-FORK-AUDIT #251 section.
...
- B. Iceberg fork audit — ✅ DONE. 21 custom commits vs Apache 1.5.2. Artifact: ICEBERG-FORK-AUDIT.md.
     #251 column defaults (api/core only: initial/write-default APIs + SchemaParser serialization; no
     read-apply and no Spark wiring in the open fork) — TABLED per owner. Tests: fork.colDefault.*.
...
- #251 column defaults — TABLED per owner (see ICEBERG-FORK-AUDIT.md #251 section).
- #249 partitioned write distribution = NONE (Apache = HASH) — not yet tested.
