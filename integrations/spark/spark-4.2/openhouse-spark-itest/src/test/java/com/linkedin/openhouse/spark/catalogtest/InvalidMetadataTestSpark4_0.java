...
/**
 * Spark-4.0 / Iceberg-1.11 / REST-first port of the 3.5 lane's {@code e2e/InvalidMetadataTest}.
 * Verifies that corrupt Iceberg metadata surfaces the real error to the client instead of being
 * masked as "Table does not exist", and that Iceberg does not delete committed files.
 *
 * <p>The 3.5 source cast the catalog to {@code OpenHouseCatalog} and used {@code newTableOps} to
 * find the metadata file. On the REST lane the catalog is a stock {@code RESTCatalog}; the metadata
 * file location is obtained via {@code ((BaseTable) table).operations().current()}. The corrupt
 * metadata still round-trips through the embedded OpenHouse server, which surfaces the invalid
 * metadata error.
 */
public class InvalidMetadataTestSpark4_0 extends OpenHouseRestSparkITest {
  ...
  void testCorruptSchemaIdSurfacesRealError() throws Exception {
    try (SparkSession spark = getSparkSession()) {
      ...
      TableOperations ops = ((BaseTable) catalog.loadTable(tableId)).operations();
