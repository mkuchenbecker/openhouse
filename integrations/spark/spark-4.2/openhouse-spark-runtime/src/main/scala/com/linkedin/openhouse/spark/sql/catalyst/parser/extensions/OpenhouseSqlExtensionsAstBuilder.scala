...
class OpenhouseSqlExtensionsAstBuilder (delegate: ParserInterface) extends OpenhouseSqlExtensionsBaseVisitor[AnyRef] {
  ...
  override def visitDuration(ctx: DurationContext): (String, Int) = {
    // Granularity tokens are the UPPERCASE names the OpenHouse server's TimePartitionSpec.Granularity
    // enum uses (and exactly what the legacy AST builder emitted via GranularityEnum.getValue()).
    // Inlined here so this Spark-4.0 module does not depend on the tables gen-model, matching the
    // `updated.openhouse.policy` contract the /iceberg REST server translates (see policy-rest-lane.md).
