...
/**
 * Pure-Java unit tests for {@link BatchedSortStatsCollectionSparkApp#buildEntries}. No Spark
 * session, no HTTP — exercises the CLI-parsing edges that decide whether the app can even start.
 */
public class BatchedSortStatsCollectionSparkAppArgsTest {

  @Test
  public void buildEntriesParsesParallelLists() {
    List<BatchedSortStatsCollectionSparkApp.BatchEntry> entries =
        BatchedSortStatsCollectionSparkApp.buildEntries(
  ...
  public void buildEntriesRejectsMismatchedLengths() {
    Assertions.assertThrows(
        ...
        () ->
            BatchedSortStatsCollectionSparkApp.buildEntries("db.a,db.b", "op-1", "uuid-1,uuid-2"));
  }
  ...
  public void buildEntriesRejectsNullOrEmptyTableNames() {
    Assertions.assertThrows(
        IllegalArgumentException.class,
        () -> BatchedSortStatsCollectionSparkApp.buildEntries(null, "op-1", "uuid-1"));
    Assertions.assertThrows(
        IllegalArgumentException.class,
        () -> BatchedSortStatsCollectionSparkApp.buildEntries("", "op-1", "uuid-1"));
  }

  @Test
  public void buildEntriesAllowsAbsentOperationIdsAndTableUuids() {
    List<BatchedSortStatsCollectionSparkApp.BatchEntry> entriesNull =
        BatchedSortStatsCollectionSparkApp.buildEntries("db.a,db.b", null, null);
    List<BatchedSortStatsCollectionSparkApp.BatchEntry> entriesEmpty =
        BatchedSortStatsCollectionSparkApp.buildEntries("db.a,db.b", "", "");

    for (List<BatchedSortStatsCollectionSparkApp.BatchEntry> entries :
  ...
  public void buildEntriesRejectsNonFqtn() {
    Assertions.assertThrows(
        IllegalArgumentException.class,
        () -> BatchedSortStatsCollectionSparkApp.buildEntries("just_a_table", "op-1", "uuid-1"));
  }

  @Test
  public void buildEntriesAcceptsAtMaxBatchSize() {
    String tableNames = generateFqtnCsv(AppConstants.SORT_STATS_COLLECTION_MAX_BATCH_SIZE);
    List<BatchedSortStatsCollectionSparkApp.BatchEntry> entries =
        BatchedSortStatsCollectionSparkApp.buildEntries(tableNames, null, null);
    Assertions.assertEquals(AppConstants.SORT_STATS_COLLECTION_MAX_BATCH_SIZE, entries.size());
  }

  @Test
  public void buildEntriesRejectsAboveMaxBatchSize() {
    String tableNames = generateFqtnCsv(AppConstants.SORT_STATS_COLLECTION_MAX_BATCH_SIZE + 1);
    IllegalArgumentException ex =
        Assertions.assertThrows(
            IllegalArgumentException.class,
            () -> BatchedSortStatsCollectionSparkApp.buildEntries(tableNames, null, null));
    Assertions.assertTrue(
        ex.getMessage().contains("SORT_STATS_COLLECTION_MAX_BATCH_SIZE"),
