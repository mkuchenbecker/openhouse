...
/**
 * Pure-Java unit tests for {@link BatchedSnapshotsExpirationSparkApp#buildEntries}. No Spark
 * session, no HTTP — exercises the CLI-parsing edges that decide whether the app can even start.
 * Mirrors {@link BatchedOrphanFilesDeletionSparkAppArgsTest}.
 */
public class BatchedSnapshotsExpirationSparkAppArgsTest {

  @Test
  public void buildEntriesParsesParallelLists() {
    List<BatchedSnapshotsExpirationSparkApp.BatchEntry> entries =
        BatchedSnapshotsExpirationSparkApp.buildEntries(
  ...
  public void buildEntriesRejectsMismatchedLengths() {
    Assertions.assertThrows(
        ...
        () ->
            BatchedSnapshotsExpirationSparkApp.buildEntries("db.a,db.b", "op-1", "uuid-1,uuid-2"));
  }
  ...
  public void buildEntriesRejectsNullOrEmptyTableNames() {
    Assertions.assertThrows(
        IllegalArgumentException.class,
        () -> BatchedSnapshotsExpirationSparkApp.buildEntries(null, "op-1", "uuid-1"));
    Assertions.assertThrows(
        IllegalArgumentException.class,
        () -> BatchedSnapshotsExpirationSparkApp.buildEntries("", "op-1", "uuid-1"));
  }
  ...
  public void buildEntriesAllowsAbsentOperationIdsAndTableUuids() {
    ...
    List<BatchedSnapshotsExpirationSparkApp.BatchEntry> entriesNull =
        BatchedSnapshotsExpirationSparkApp.buildEntries("db.a,db.b", null, null);
    List<BatchedSnapshotsExpirationSparkApp.BatchEntry> entriesEmpty =
        BatchedSnapshotsExpirationSparkApp.buildEntries("db.a,db.b", "", "");

    for (List<BatchedSnapshotsExpirationSparkApp.BatchEntry> entries :
  ...
  public void buildEntriesRejectsNonFqtn() {
    Assertions.assertThrows(
        IllegalArgumentException.class,
        () -> BatchedSnapshotsExpirationSparkApp.buildEntries("just_a_table", "op-1", "uuid-1"));
  }

  @Test
  public void buildEntriesAcceptsAtMaxBatchSize() {
    String tableNames = generateFqtnCsv(AppConstants.SNAPSHOTS_EXPIRATION_MAX_BATCH_SIZE);
    List<BatchedSnapshotsExpirationSparkApp.BatchEntry> entries =
        BatchedSnapshotsExpirationSparkApp.buildEntries(tableNames, null, null);
    Assertions.assertEquals(AppConstants.SNAPSHOTS_EXPIRATION_MAX_BATCH_SIZE, entries.size());
  }

  @Test
  public void buildEntriesRejectsAboveMaxBatchSize() {
    String tableNames = generateFqtnCsv(AppConstants.SNAPSHOTS_EXPIRATION_MAX_BATCH_SIZE + 1);
    IllegalArgumentException ex =
        Assertions.assertThrows(
            IllegalArgumentException.class,
            () -> BatchedSnapshotsExpirationSparkApp.buildEntries(tableNames, null, null));
    Assertions.assertTrue(
        ex.getMessage().contains("SNAPSHOTS_EXPIRATION_MAX_BATCH_SIZE"),
